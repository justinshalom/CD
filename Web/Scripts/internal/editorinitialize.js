...
function require(category, name) {
    ...
    requesthandler( window.cd_scripturl + "/" + category + '.js',false,"getScript");
}
function include(category, name) {
    if (name) {
        category = category + "/" + name;
    }
    $('head').prepend('<link href="' + window.cd_styleurl + "/" + category + '.css" rel="stylesheet" />');
}
$(document).ready(function () {
  
    require("internal", "localcache");
    ...
    if (window.getcache("editormode") == "design") {
    ...
    if (window.getcache("editormode") == "db") {
